...
def test_calcular_media_quatro_notas(aluno_aprovado):
    # notas = [8, 9, 7, 8] -> soma 32 / 4 notas = 8.0
    assert aluno_aprovado.calcular_media() == 8.0


def test_calcular_media_tres_notas():
...
def test_situacao_media_seis_aprova():
...
# --- maior_nota não tinha bug, mas estava sem cobertura de teste ---

def test_maior_nota(aluno_aprovado):
    # notas = [8, 9, 7, 8] -> a maior é 9
    assert aluno_aprovado.maior_nota() == 9
...
def test_media_arredondada_quatro_notas(aluno_reprovado):
    # notas = [4, 3, 5, 4] -> soma 16 / 4 = 4.0 -> arredondado 4
    assert aluno_reprovado.calcular_media_arredondada() == 4


def test_media_arredondada_arredonda_pra_cima():
...
def test_contar_aprovados_mistos():
    alunos = [
        Aluno(nome="A", notas=[8, 8, 8, 8]),
...
def test_situacao_final_falta_acima_25_reprova():
...
def test_enviar_boletim_reprovado_aciona_email(aluno_reprovado):
...
def test_enviar_boletim_aprovado_nao_aciona_email(aluno_aprovado):
    email_service = MagicMock()
